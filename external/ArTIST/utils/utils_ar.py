...
def quantize_cluster(delta_x, delta_y, delta_w, delta_h, centroid_x, centroid_y, centroid_w, centroid_h, num_cluster):
    ...
    min_idx_list_x = (centroid_x.squeeze() == centroid_x[0, min_idx_x.squeeze()]).nonzero(as_tuple=True)
    ...
    min_idx_list_y = (centroid_y.squeeze() == centroid_y[0, min_idx_y.squeeze()]).nonzero(as_tuple=True)
    ...
    min_idx_list_w = (centroid_w.squeeze() == centroid_w[0, min_idx_w.squeeze()]).nonzero(as_tuple=True)
    ...
    min_idx_list_h = (centroid_h.squeeze() == centroid_h[0, min_idx_h.squeeze()]).nonzero(as_tuple=True)

    return min_idx_x, min_idx_y, min_idx_w, min_idx_h, min_idx_list_x, min_idx_list_y, min_idx_list_w, min_idx_list_h
...
def infer_log_likelihood(dist_x, dist_y, dist_w, dist_h, x, y, w, h, centroid_x, centroid_y, centroid_w, centroid_h, num_clusters):
    probs = []
    idx_x, idx_y, idx_w, idx_h, min_idx_list_x, min_idx_list_y, min_idx_list_w, min_idx_list_h = quantize_cluster(x[:, -1:, :], y[:, -1:, :], w[:, -1:, :], h[:, -1:, :], centroid_x, centroid_y, centroid_w, centroid_h, num_clusters)  ## input last_delta and centroids
    ...
    prob_x = dist_x[0, -1, :][min_idx_list_x].sum()
    prob_y = dist_y[0, -1, :][min_idx_list_y].sum()
    prob_w = dist_w[0, -1, :][min_idx_list_w].sum()
    prob_h = dist_h[0, -1, :][min_idx_list_h].sum()
    prob_t = [prob_x, prob_y, prob_w, prob_h]

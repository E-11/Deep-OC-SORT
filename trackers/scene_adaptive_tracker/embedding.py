...
class EmbeddingComputer:
    ...
    def initialize_model(self):
        if self.dataset == "mot17":
            if self.test_dataset:
                path = os.path.join(model_path, "mot17_sbs_S50.pth")
            else:
                return self._get_general_model()
        elif self.dataset == "mot20":
            if self.test_dataset:
                path = os.path.join(model_path, "mot20_sbs_S50.pth")
            else:
                return self._get_general_model()
        elif self.dataset == "dance":
            path = os.path.join(model_path, "dance_sbs_S50.pth")
        else:
    ...
    def _get_general_model(self):
        ...
        sd = torch.load(os.path.join(model_path, "osnet_ain_ms_d_c.pth.tar"))["state_dict"]

...
def compute_scene_adaptive_weight(detections, tracks_ls):
    '''
    params:
        track_ls: [track_len, track_state]
    '''
    N = len(detections)
    M = len(tracks_ls)

    ## w_iou
    det_iou_matrix = iou_batch(detections, detections)  ## shape=N×N
    ...
    ## w_length
    w_length = np.zeros((len(tracks_ls),))  ## (M,)
    for t, trk in enumerate(tracks_ls):
        if trk[1]:  ## trk.frozen == True
            w_length[t] = 0
        else:
            w_length[t] = 1 / trk[0]
    w_length = np.expand_dims(w_length, axis=0)  ## 1×M
    w_length = w_length.repeat(N, axis=0)  ## N×M
...
def associate(
    detections,
    trackers,  ## predicted locations of existing trackers
    tracks_len_state,
    det_embs,
    trk_embs,
    iou_threshold,
    ...
    if min(iou_matrix.shape) > 0:
        ...
        if a.sum(1).max() == 1 and a.sum(0).max() == 1:
            ...
            final_cost = -(iou_emb_cost + angle_diff_cost)
